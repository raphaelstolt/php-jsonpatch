...
If necessary you can disable some patch operations by setting a whitelist bitmask in the constructor e.g. `$document = new Document($patchDocument, Add::APPLY | Copy::APPLY | Replace::APPLY | Remove::APPLY);`. This will not allow the usage the `move` or `test` patch operation in the document. If used, these operations are just ignored. The default is to allow all patch operations.
